...
void MainWindow::init()
{
    ...
    /*========全屏/关闭/缩小=========*/
    // ========== 1️⃣ 先拿到 centralWidget 和已有 layout ==========
    ...
    // ========== 3️⃣ 把 titleBar 插到 centralLayout 最顶端 ==========
    ...
    // centralLayout->addWidget(ui->VideoWidget);
    // 初始化 QAudioProbe
   ...
   // 获取UI中的QSplitter对象，假设它的objectName是splitter
    ...
    //设置分隔条初始大小
    splitter->setSizes({200, 500});  // 左侧400px，右侧300px

    //设置最小尺寸
    // ui->funcWidget->setMinimumSize(200, 200);  // 设置视频区域最小尺寸
    // ui->RU_Widget->setMinimumSize(150, 200);  // 设置listWidget的最小尺寸

    //设置sizePolicy来确保控件大小可自适应
    ui->funcWidget->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
    ui->RU_Widget->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
...
void MainWindow::on_comboBox_theme_currentIndexChanged(int index)
{
    ...
    case COLORFUL:
        ...
        break;
    case FOLLOW_EMOTION:
        installPath = "/opt/apps/org.xuyue/files/bin/library/py/followModel.pkl";
        //qicPath = ":/qic/colorPre/followModel.pkl";

        // 如果安装目录下的文件存在，优先使用该路径
        //if (QFile::exists(installPath)) {
            modelPath = installPath;
        //}
        //modelPath = extractResourceToTempFile(qicPath);
        qDebug() << "followmodel.pkl path:" << modelPath;
        emotion_to_theme(modelPath);
        break;
    case REVERSE_EMOTION:
        installPath = "/opt/apps/org.xuyue/files/bin/library/py/reverseModel.pkl";
        //qicPath = ":/qic/colorPre/reverseModel.pkl";
        ...
        //}
        //modelPath = extractResourceToTempFile(qicPath);
        qDebug() << "revermodel.pkl path:" << modelPath;
        emotion_to_theme(modelPath);
        break;
    case AUTO_EMOTION:
        installPath = "/opt/apps/org.xuyue/files/bin/library/py/model.pkl";
        //qicPath = ":/qic/colorPre/model.pkl";

        // 如果安装目录下的文件存在，优先使用该路径
        //if (QFile::exists(installPath)) {
            modelPath = installPath;
        //}
        qDebug() << "auto model.pkl path:" << modelPath;
        //modelPath = extractResourceToTempFile(qicPath);
        emotion_to_theme(modelPath);
        break;
    }
...
void MainWindow::emotion_to_theme(const QString &modelPath)
{
    ...
    QString pythonExecutable = "/opt/apps/org.xuyue/files/bin/library/venv/bin/python3";  // 指定虚拟环境 Python
    // 设置环境变量
    QProcessEnvironment env = QProcessEnvironment::systemEnvironment();
    env.insert("PYTHONPATH", "/opt/apps/org.xuyue/files/bin/library/py:/opt/apps/org.xuyue/files/bin/library/venv/lib/python3.10/site-packages");
    env.insert("VIRTUAL_ENV", "/opt/apps/org.xuyue/files/bin/library/venv");  // 让 Python 识别 venv
    env.insert("PATH", "/opt/apps/org.xuyue/files/bin/library/venv/bin:" + env.value("PATH"));  // 确保 venv 里的 python 可用

    process.setProcessEnvironment(env);

    QString scriptPath;
    QString installedPath = "/opt/apps/org.xuyue/files/bin/library/py/colorChoose.py";
    //QString resourcePath = ":/qic/colorPre/colorChoose.py";

    // 如果安装目录下的文件存在，优先使用该路径
    // if (QFile::exists(installedPath)) {
        scriptPath = installedPath;
    // }
    // scriptPath = extractResourceToTempFile(resourcePath);

    //QString model_path=extractResourceToTempFile(modelPath);
        qDebug() << "colorChoose.py path:" << scriptPath;
...
void MainWindow::on_btn_shrink_expand_clicked()
{
    ...
    // static int initialWidth = sizes[0] > 0 ? sizes[0] : 200;

    // if (sizes[0] > 0) {  // right_widget 处于展开状态
    //     sizes[0] = 0;  // 收缩 right_widget
    //     ui->btn_shrink_expand->setText("<");  // 改为“展开”状态
    // } else {  // right_widget 处于收缩状态
    //     sizes[0] = initialWidth;  // 恢复到原来的宽度
...
void MainWindow::start_voice_to_text() {
    ...
    // // 使用 venv 里的 Python 解释器
    // QString program = "/opt/apps/org.xuyue/files/bin/library/venv/bin/python3";

    // // 确保 Python 解释器存在
    // if (!QFile::exists(program)) {
    //     qDebug() << "Error: Virtual environment Python not found!";
    //     return;
    // }

    // QStringList arguments;
    // QString starPath;
    // QString installedPath = "/opt/apps/org.xuyue/files/bin/library/py/star.py";
    // //QString resourcePath = ":/qic/star.py";

    // // 优先使用安装目录下的 star.py
    // // if (QFile::exists(installedPath)) {
    //     starPath = installedPath;
    // // } else {
    // //     // 否则，从资源文件提取到临时目录
    // //     starPath = extractResourceToTempFile(resourcePath);
    // //     if (starPath.isEmpty()) {
    // //         qDebug() << "Error: Failed to extract star.py from resources!";
    // //         return;
    // //     }
    // // }
    // qDebug() << "star.py path:" << starPath;
    // arguments << starPath << filePath;

    // QProcess *process = new QProcess(this);

    // // 设置 Python 运行环境，确保只使用 venv
    // QProcessEnvironment env = QProcessEnvironment::systemEnvironment();
    // env.insert("PYTHONPATH", "/opt/apps/org.xuyue/files/bin/library/py:/opt/apps/org.xuyue/files/bin/library/venv/lib/python3.10/site-packages");
    // env.insert("VIRTUAL_ENV", "/opt/apps/org.xuyue/files/bin/library/venv");
    // env.insert("PATH", "/opt/apps/org.xuyue/files/bin/library/venv/bin:" + env.value("PATH"));
    // process->setProcessEnvironment(env);
